...
function useExcelCache(tableData, setTableData) {
  ...
  useEffect(() => {
    try {
      const savedData = localStorage.getItem("exceslTableData");
      const savedKeys = localStorage.getItem("excelsSelectedKeys");
      ...
      if (savedData && savedData !== "undefined" && savedData !== "null") {
        ...
        localStorage.removeItem("exceslTableData");
        localStorage.removeItem("excelsSelectedKeys");
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    try {
      if (Array.isArray(tableData)) {
        localStorage.setItem("exceslTableData", JSON.stringify(tableData));
      }
    } catch (e) {
      console.error("Save exceslTableData failed:", e);
    }
  ...
  useEffect(() => {
    if (!isInitialized) return;
    try {
      localStorage.setItem("excelsSelectedKeys", JSON.stringify(selectedRowKeys));
    } catch (e) {
  ...
  const handleFileUpload = (file) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        localStorage.setItem("exceslTableData", JSON.stringify(dataWithId));
